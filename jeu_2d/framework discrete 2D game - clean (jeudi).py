...
class environnement():
    ...
    def __init__(self, n, rewarded_states=[], rewards=[], terminal_states=[], forbidden_states=[], start_point=[0, 0], max_plays = 30, move_penalty = -1):
        ...
        self.frame = Frame(self.root, height = 80, width = size + 1, bg='red')
        self.frame.pack()
...
E=environnement(n=7, rewarded_states=[[0,0],[1,0],[6,0],[1,1],[5,1],[4,2],[4,6],[5,6]], terminal_states=[[1,0],[6,0],[1,1],[4,2]], rewards=[100, -1000, -1000, -1000, 10, -1000, 2, 2], start_point=[1, 5])    
...
successivement, ainsi qu'une QTable évolutive."""
    ...
    def turn_tempo(self): #methode pour interaction via IHM
    ...
    def show_arrows(self):
        ...
        if 'Left' in self.G.best_actions():
            self.env.canvas.create_line(2+(i+0.25)*self.env.step, 2+(j+0.5)*self.env.step,2+(i-0.25)*self.env.step, 2+(j+0.5)*self.env.step, arrow='last', tag='flèche_%d_%d'%(i, j))
        if 'Right' in self.G.best_actions():
            self.env.canvas.create_line(2+(i+0.75)*self.env.step, 2+(j+0.5)*self.env.step,2+(i+1.25)*self.env.step, 2+(j+0.5)*self.env.step, arrow='last', tag='flèche_%d_%d'%(i, j))
        if 'Up' in self.G.best_actions():
            self.env.canvas.create_line(2+(i+0.5)*self.env.step, 2+(j+0.25)*self.env.step,2+(i+0.5)*self.env.step, 2+(j-0.25)*self.env.step, arrow='last', tag='flèche_%d_%d'%(i, j))            
        if 'Down' in self.G.best_actions():
            self.env.canvas.create_line(2+(i+0.5)*self.env.step, 2+(j+0.75)*self.env.step,2+(i+0.5)*self.env.step, 2+(j+1.25)*self.env.step, arrow='last', tag='flèche_%d_%d'%(i, j))
        return
