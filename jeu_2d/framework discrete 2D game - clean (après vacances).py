...
def new_player():
    global player1
    player1=player()#réinitialise le joueur
    return
...
button_train=Button(frame, command=lambda: player1.train(), text='Entraînement') #obligé de passer par lambda: player1.train() au lieu de juste player1.train
button_train.pack()                                                 #sinon la définition du bouton garde en mémoire le premier objet player1 même après new_player()

button_switch_tempo=Button(frame, command=lambda: player1.switch_tempo(), text='Temporisation')
button_switch_tempo.pack()

button_play=Button(frame, command=lambda: player1.play(), text='Jouer une partie optimale')
button_play.pack()

button_alea=Button(frame, command=lambda: player1.play(method='e_greedy', epsilon=1), text='Jouer une partie aléatoire')
button_alea.pack()
